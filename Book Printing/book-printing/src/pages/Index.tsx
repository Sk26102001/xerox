...
// // Add this new component for customer testimonial submission
// function CustomerTestimonialForm({ onSuccess }: { onSuccess: () => void }) {
...
//       toast.error('Please enter your name and testimonial');
//       return;
//     }
...
//       // Make sure your backend has a public route for this
//       const response = await API.post('/testimonials/public', {     
//         name: formData.name.trim(),
//         role: formData.role.trim(),
//         content: formData.content.trim(),
//         rating: formData.rating,
//         active: false // New testimonials need admin approval
//       });

//       if (response.data.success) {
//         toast.success('Thank you for your testimonial! It will appear after admin approval.');
...
//         </div>
//         <div>
//           <label className="block text-sm font-medium text-foreground mb-1">Role / Title (Optional)</label>
//           <input
//             type="text"
//             value={formData.role}
//             onChange={(e) => setFormData({ ...formData, role: e.target.value })}
//             placeholder="e.g., Author, Student, Business Owner"
//             className="w-full px-4 py-2 border border-border rounded-lg focus:ring-2 focus:ring-primary focus:border-primary outline-none"
//           />
//         </div>
//       </div>
...
//       <div className="mb-4">
//         <label className="block text-sm font-medium text-foreground mb-1">Your Testimonial *</label>
//         <textarea
//           value={formData.content}
//           onChange={(e) => setFormData({ ...formData, content: e.target.value })}
//           placeholder="Share your experience with our printing services..."
//           rows={3}
//           className="w-full px-4 py-2 border border-border rounded-lg focus:ring-2 focus:ring-primary focus:border-primary outline-none resize-none"
//           required
//         />
//       </div>
...
//             >
//               <Star 
//                 className={`h-8 w-8 ${r <= formData.rating ? 'text-yellow-500 fill-yellow-500' : 'text-gray-300'}`}
//               />
//             </button>
//           ))}
//         </div>
...
//         type="submit"
//         disabled={submitting}
//         className="w-full bg-primary text-white font-semibold py-3 rounded-lg hover:bg-primary/90 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
//       >
...
//       {/* Updated Testimonials Section with dynamic data and submission form */}
//       <section className="py-20 bg-background">
//         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
//           <div className="text-center mb-12 animate-on-scroll">
...
//                   <span className="font-semibold">{stats.avgRating} out of 5</span>
//                 </div>
...
//                     {item.role && <p className="text-muted-foreground text-sm">{item.role}</p>}
...
//             <CustomerTestimonialForm onSuccess={fetchTestimonials} />
//           </div>
//         </div>
//       </section>
...
export default function Index() {
  ...
  const fetchTestimonials = async () => {
    try {
      ...
      if (response.data.success) {
        const testimonialsData = Array.isArray(response.data.data) ? response.data.data : [];
      ...
      console.error('🔴 Error fetching testimonials:', error);
      setError(error.response?.data?.error || 'Failed to load testimonials');
      setTestimonials([]);
    } finally {
      setLoadingTestimonials(false);
    }
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative bg-secondary min-h-screen flex items-center overflow-hidden pt-16">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-8">
              <div className="inline-flex items-center gap-2 bg-primary/20 text-white border border-primary/30 rounded-full px-4 py-2 text-sm font-medium animate-slide-up">
                <Star className="h-4 w-4 text-primary" />
                India's Most Trusted Printing Partner
              </div>
              <h1 className="animate-slide-up text-4xl sm:text-5xl lg:text-6xl font-black text-white leading-tight" style={{ animationDelay: '0.1s' }}>
              ...
              <div className="animate-slide-up flex flex-col sm:flex-row gap-4" style={{ animationDelay: '0.3s' }}>
                <Link to="/order" className="btn-pulse flex items-center justify-center gap-2 bg-primary text-white font-bold px-8 py-4 rounded-lg text-lg hover:bg-primary/90 transition-all duration-200 hover:scale-105 active:scale-95">
                  Order Now <ArrowRight className="h-5 w-5" />
                </Link>
                <a href="#calculator" className="flex items-center justify-center gap-2 border-2 border-white/30 text-white font-bold px-8 py-4 rounded-lg text-lg hover:bg-white/10 transition-all duration-200 hover:border-white">
                  Get a Quote <Calculator className="h-5 w-5" />
                </a>
              </div>
      ...
      <section className="py-20 bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12 animate-on-scroll">
            <span className="text-primary font-semibold text-sm uppercase tracking-widest">Our Services</span>
            <h2 className="text-3xl sm:text-4xl font-black text-foreground mt-2">Everything You Need to Print</h2>
            <p className="text-muted-foreground mt-3 max-w-xl mx-auto">From single page photocopies to bulk book printing — we handle it all with precision and care.</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {services.map((service, i) => (
              <div key={service.title} className="animate-on-scroll card-hover bg-white rounded-xl overflow-hidden border border-border shadow-sm group cursor-pointer flex flex-col hover:border-primary" style={{ transitionDelay: `${i * 0.1}s` }}>
                <div className="h-48 overflow-hidden">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <h3 className="font-bold text-foreground text-lg mb-2">{service.title}</h3>
                  <p className="text-muted-foreground text-sm leading-relaxed flex-grow">{service.desc}</p>
                  <Link to="/order" className="inline-flex items-center gap-1 text-primary text-sm font-semibold mt-4 hover:gap-2 transition-all duration-200">
                    Order Now <ChevronRight className="h-4 w-4" />
                  </Link>
                </div>
              </div>
      ...
      <section className="py-20 bg-muted/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
              {steps.map((step, i) => (
                <div key={step.step} className="animate-on-scroll text-center group" style={{ transitionDelay: `${i * 0.15}s` }}>
                  <div className="relative inline-flex">
                    <div className="w-20 h-20 bg-secondary rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:bg-primary transition-all duration-300 group-hover:scale-110 shadow-lg">
                      <step.icon className="h-8 w-8 text-white" />
                    </div>
      ...
      <section className="py-16 md:py-20 bg-gradient-to-b from-background to-muted/30 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-[16px] font-bold uppercase tracking-wider text-primary mb-5">
              <BookOpen className="h-5 w-4" /> Gallery
            </div>
            <p className="mt-4 text-muted-foreground text-lg max-w-3xl mx-auto">From high-speed digital presses to expert binding — see how your books come to life with precision and care</p>
          </div>
          
          <div className="relative overflow-hidden">
            <div className="flex gap-6 md:gap-8 animate-scroll hover:animation-pause">
              {galleryImages.map((image, index) => (
                <div key={index} className="min-w-[280px] sm:min-w-[340px] md:min-w-[400px] h-64 md:h-80 flex-shrink-0 rounded-xl md:rounded-2xl overflow-hidden shadow-xl border border-border/40 bg-muted/20 group transition-all duration-500 hover:shadow-2xl hover:scale-[1.02]">
                  <img src={image.url} alt={image.alt} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" loading="lazy" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="animate-on-scroll bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {[
                ...
                <div key={field.label} className={field.label === 'Email Address' ? 'sm:col-span-2' : ''}>
                  <label className="block text-white/70 text-sm font-medium mb-1">{field.label}</label>
                  <input type={field.type} placeholder={field.placeholder} className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/30 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all text-sm" required />
                </div>
              ))}
            </div>
            <button type="submit" className="w-full mt-8 bg-primary text-white font-bold py-4 rounded-lg hover:bg-primary/90 transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] text-lg">
              Send Quotation Request →
            </button>
          </div>
        </div>
      ...
      <section className="py-20 bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-12 rounded-2xl overflow-hidden shadow-2xl">
            <img src="/img1.jpeg" alt="High-end digital printing production line" className="w-full h-80 sm:h-96 object-cover" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: Award, title: 'Best Quality', desc: 'High-resolution printing with premium paper options that make your books stand out.' },
              ...
              <div key={item.title} className="animate-on-scroll flex gap-4 p-6 bg-white rounded-xl border border-border hover:border-primary hover:shadow-md transition-all duration-300" style={{ transitionDelay: `${i * 0.1}s` }}>
                <div className="bg-primary/10 w-12 h-12 rounded-lg flex items-center justify-center shrink-0">
                  <item.icon className="h-6 w-6 text-primary" />
                </div>
                <div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {testimonials.length > 0 && (
              <div className="flex flex-wrap justify-center gap-6 mt-4">
                <div className="flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-full">
                  ...
                  <span className="text-xs text-muted-foreground">({testimonials.length} reviews)</span>
                </div>
              </div>
            )}
          </div>
          ...
          {loadingTestimonials ? (
            ...
            <div className="text-center py-8">
              ...
              <button onClick={fetchTestimonials} className="text-primary underline">Try Again</button>
            </div>
          ) : testimonials.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              No testimonials yet. Be the first to share your experience!
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
              {testimonials.map((item) => (
                <div key={item._id} className="flex flex-col gap-4 p-6 bg-white rounded-xl border border-border hover:border-primary hover:shadow-md transition-all duration-300">
                  <div className="flex items-start justify-between">
                    <div className="bg-primary/10 w-12 h-12 rounded-lg flex items-center justify-center shrink-0">
                      <Quote className="h-6 w-6 text-primary" />
                    </div>
                  ...
                  <p className="text-muted-foreground italic leading-relaxed flex-1">"{item.content}"</p>
                  <div className="mt-2">
                    <h3 className="font-bold text-foreground">{item.name}</h3>
                    {item.role && <p className="text-muted-foreground text-sm">{item.role}</p>}
      ...
      <section className="py-16 bg-gradient-to-br from-secondary to-secondary/90">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center animate-on-scroll">
          <Instagram className="h-12 w-12 text-primary mx-auto mb-4 animate-float" />
          <h2 className="text-2xl sm:text-3xl font-black text-white mb-3">Follow Us on Instagram</h2>
          <p className="text-white/60 mb-6">See our printed books, customer stories and behind-the-scenes at Shree Publication</p>
          <a href="https://www.instagram.com/shree_publication?igsh=MXhmY3l5dWowN24zeg==" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500 text-white font-bold px-8 py-4 rounded-xl hover:opacity-90 transition-all duration-200 hover:scale-105 active:scale-95 text-lg">
            <Instagram className="h-5 w-5" />
            @shree_publication
          </a>
        </div>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        ...
        <a href="https://wa.me/917230001405" target="_blank" rel="noopener noreferrer" className="relative inline-flex items-center justify-center bg-[#25D366] text-white p-3 rounded-full shadow-md hover:scale-110 transition-transform duration-300">
          <FaWhatsapp className="h-9 w-9" />
        </a>
      </div>
