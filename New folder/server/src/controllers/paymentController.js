...
export const createRazorpayOrder = async (req, res) => {
    try {
        ...
        // ✅ Razorpay options WITH customer object
        const options = {
            amount: Math.round(paymentAmount * 100),
            currency: 'INR',
            receipt: `order_${orderId}`,
            customer: customerDetails,   // <-- CRITICAL FIX
            notes: {
                orderId: orderId.toString(),
                userId: userId.toString(),
                orderNumber: order.orderNumber || `ORD-${orderId}`,
...
export const getPaymentStatus = async (req, res) => {
    try {
        ...
        // ✅ First, check the order's payment status directly from the Order collection
        const order = await Order.findById(orderId);
        
        console.log('Order found:', order ? order.orderNumber : 'NO');
        console.log('Order paymentStatus from DB:', order?.paymentStatus);
        ...
        // ✅ Also check if order exists but payment is pending
        if (order && order.paymentStatus === 'pending') {
            console.log(`⏳ Order ${order.orderNumber} payment status is still pending`);
        }
        ...
        if (!payment) {
            ...
            // Return pending status instead of 404
            return res.status(200).json({
                success: true,
                payment: {
                    status: order?.paymentStatus || 'pending',
                    amount: (order?.finalAmount || order?.totalAmount || 0) * 100,
                    currency: 'INR',
                    createdAt: order?.createdAt,
                    orderDetails: order ? {
                        _id: order._id,
        ...
        console.log(`Payment status from payment collection: ${payment.status}`);
