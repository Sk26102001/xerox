...
// ✅ NEW: Get dynamic binding label with actual cost
export const getBindingLabelWithCost = (bindingType: BindingType, copies: number): string => {
  const baseLabel = bindingLabels[bindingType];
  const bindingCost = defaultPricingConfig.bindingPrices[bindingType] ?? 0;
  const totalBindingCost = bindingCost * copies;
  
  if (bindingCost === 0) {
    return baseLabel;
  }
  
  return `${baseLabel.split('(+')[0]} (+₹${totalBindingCost.toFixed(2)})`;
};

// ✅ NEW: Get binding display for summary (shows per copy and total)
export const getBindingDisplay = (bindingType: BindingType, copies: number): { name: string; perCopyCost: number; totalCost: number } => {
  const perCopyCost = defaultPricingConfig.bindingPrices[bindingType] ?? 0;
  const totalCost = perCopyCost * copies;
  const name = bindingLabels[bindingType].split('(+')[0].trim();
...
// ✅ NEW: Get binding cost breakdown for display in order summary
export const getBindingBreakdown = (bindingType: BindingType, copies: number): string => {
  const perCopyCost = defaultPricingConfig.bindingPrices[bindingType] ?? 0;
  const totalCost = perCopyCost * copies;
  ...
  // return `${bindingLabels[bindingType].split('(+')[0]} (₹${perCopyCost} × ${copies} = ₹${totalCost})`;
    return `${bindingLabels[bindingType].split('(+')[0]} (₹${perCopyCost} × ${copies} = ₹${totalCost})`;
};
