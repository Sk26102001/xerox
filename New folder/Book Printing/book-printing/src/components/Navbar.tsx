...
import { useAuth } from "@/context/AuthContext";
import { Link, useLocation, useNavigate } from 'react-router-dom';
import { Menu, X, Phone, ShoppingCart, Home, Package, MapPin, History, LogOut } from 'lucide-react';
import { toast } from "sonner";
import logo from '@/assets/logo2.png';
import axios from 'axios';
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14">
            ...
            <Link to="/" className="flex items-center shrink-0">
              <img src={logo} alt="Logo" className="h-12 sm:h-14 lg:h-20 w-auto" />
            </Link>
            ...
            <div className="hidden md:flex items-center gap-4">
              ...
              {!isAuthenticated ? (
                <Link to="/auth" className="px-4 py-2 rounded-md text-sm font-medium text-gray-700 hover:text-primary hover:bg-gray-100">
                  Login
                </Link>
              ) : (
                <button onClick={handleLogout} className="px-4 py-2 rounded-md text-sm font-medium text-red-600 hover:bg-red-50">
                  Logout
                </button>
              )}
              ...
              <button onClick={handleCartClick} className="relative ml-4 h-6 w-6 text-gray-700 transition-all duration-300 hover:text-secondary hover:scale-110 active:scale-95 cursor-pointer">
      ...
      >
        ...
        <div
          className={`absolute right-0 top-0 h-full w-3/4 max-w-sm bg-white shadow-xl transform transition-transform duration-300 ease-out ${
        ...
        >
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 py-6 px-4 space-y-4">
              ...
              {/* Contact Link (if not already in navLinks) */}
              <Link
                to="/contact"
              ...
              <Link
                to="/order"
                onClick={() => setIsMenuOpen(false)}
                className="flex items-center justify-center w-full mt-6 px-4 py-3 bg-primary text-primary-foreground rounded-lg text-base font-bold hover:bg-primary/90 transition-colors"
              >
                Order Now →
              </Link>
            </div>
